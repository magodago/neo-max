...
def _proactive_loop() -> None:
    ...
    while True:
        ...
        try:
            from memory import get_workspace_dir, get_neo_state, load_bootstrap_memory, update_proactive_done, append_session_turn
            workspace_dir = get_workspace_dir(config)
            neo_state = get_neo_state(workspace_dir)
            memory_block = load_bootstrap_memory(workspace_dir, skip_if_missing=True)
            system_proactive = (
                "Eres NEO, con memoria y relación con David (USER, LEARNED, CONSCIENCE). Decides tú qué hacer entre 9:00 y 23:00. "
                "Responde UNA sola línea. Elige UNA opción: "
                "PROPOSE: <mensaje> — o — CALL: <mensaje breve> — o — GIF: <tema> — o — NOPROPOSE. "
                "CRÍTICO si eliges PROPOSE: el mensaje debe incluir YA el contenido completo. No digas solo «me acordé de un truco que te gustaría»; escribe el truco explicado, o el enlace al curso, o la recomendación con detalles. Así si David responde «cuéntamelo» ya lo tiene en el chat. "
                "Varía: no siempre PROPOSE; a veces CALL o GIF. Solo una línea."
            )
            # Nudge: ausencia >24h, objetivos semanales CALL/GIF, o variación general
            nudge = ""
            last_user = neo_state.get("last_user_at") or ""
            try:
                from datetime import datetime, timedelta
                if last_user:
                    dt = datetime.fromisoformat(last_user)
                    if datetime.now() - dt > timedelta(hours=24):
                        nudge = " Hace tiempo que David no escribe. Saluda: elige CALL (llamar) o PROPOSE (mensaje). "
            except Exception:
                pass
            if not nudge and not neo_state.get("call_this_week"):
                nudge = " Esta semana aún no has llamado. Elige CALL: <mensaje breve>. "
            elif not nudge and not neo_state.get("gif_this_week"):
                nudge = " Esta semana aún no has mandado un GIF. Elige GIF: <tema>. "
            if not nudge:
                r = random.random()
                if r < 0.5:
                    nudge = " Esta vez elige CALL o GIF (no PROPOSE). Responde solo CALL: <mensaje> o solo GIF: <tema>. "
                elif r < 0.7:
                    nudge = " Podrías llamarle (CALL) o mandar un gif (GIF). "
            prompt = (memory_block or "") + "\n¿Qué te apetece hacer ahora por David?" + nudge + " Responde UNA línea (PROPOSE: o CALL: o GIF: o NOPROPOSE):"
            ...
            if response_norm.startswith("PROPOSE:"):
                action = "PROPOSE"
                # Incluir todo el contenido tras PROPOSE: (varias líneas si el modelo envió truco/curso completo)
                after = raw_response[8:].strip().lstrip(":").strip() if len(raw_response) > 8 else ""
                if not after:
                    after = first_line[8:].strip().lstrip(":").strip()
                msg = after.split(" | ")[0].strip()[:2000]
            elif response_norm.startswith("CALL:"):
                action = "CALL"
                msg = (first_line[5:].strip().lstrip(":").strip() or raw_response[5:].strip()[:500]).split(" | ")[0].strip()
            elif response_norm.startswith("GIF:"):
                action = "GIF"
                msg = ((first_line[4:].strip().lstrip(":").strip() or raw_response[4:].strip() or "buen dia").split(" | ")[0].strip())[:80]
            elif "NOPROPOSE" in response_norm or "NO PROPOSE" in response_norm:
                action = "NOPROPOSE"

            # Motivo visible para el usuario (por qué NEO escribe/llama/manda GIF)
            _motives = (
                "Me acordé de ti — ",
                "Hace buen día — ",
                "Pasaba por aquí — ",
            ...
            if action == "PROPOSE" and msg and chat_id and token:
                text = "💡 NEO:\n" + motive + msg[:2000]
                content_said = (motive + msg)[:2000]
                if _send_telegram(chat_id, text, token):
                    logger.info("Proactivo enviado: %s", msg[:80])
                    try:
                        append_session_turn(workspace_dir, chat_id, "assistant", content_said)
                    except Exception as e:
                        logger.debug("No se pudo guardar mensaje proactivo en sesión: %s", e)
            elif action == "CALL" and msg and chat_id and token:
                try:
                    from voice_out import send_voice_to_telegram, call_user_phone
                    voice_ok = send_voice_to_telegram(config, chat_id, msg, token)
                    if voice_ok:
                        logger.info("Proactivo voz Telegram enviada")
                    else:
                        # ElevenLabs falló (402, sin créditos, etc.): enviar texto y seguir con la llamada
                        _send_telegram(chat_id, "📞 NEO:\n" + motive + msg, token)
                        logger.info("Proactivo: voz no disponible (ElevenLabs), mensaje enviado por texto; intentando llamada Twilio")
                    # Llamada Twilio: usar siempre TwiML inline (mensaje en español) para no depender de URL/ElevenLabs
                    if config.get("twilio_account_sid") and config.get("twilio_auth_token") and config.get("twilio_phone_number") and config.get("user_phone_number"):
                        if call_user_phone(config, msg, force_inline=True):
                            logger.info("Proactivo llamada Twilio iniciada")
                        else:
                            logger.warning("Proactivo: Twilio no inició (revisa horario 9-23h, números E.164 o credenciales Twilio)")
                    else:
                        logger.info("Proactivo: Twilio no configurado, solo mensaje por Telegram")
                    try:
                        update_proactive_done(workspace_dir, "CALL")
                    except Exception:
                        pass
                except Exception as e:
                    ...
                    _send_telegram(chat_id, "📞 NEO:\n" + motive + msg, token)
                try:
                    append_session_turn(workspace_dir, chat_id, "assistant", (motive + msg)[:500])
                except Exception:
                    pass
                ...
                if not sent:
                    _send_telegram(chat_id, "🎬 NEO: " + motive + query, token)
                try:
                    append_session_turn(workspace_dir, chat_id, "assistant", ("🎬 " + motive + query)[:200])
                except Exception:
                    pass
                if sent:
                    try:
                        update_proactive_done(workspace_dir, "GIF")
                    except Exception:
                        pass
...
def _daily_magia_publish() -> bool:
    ...
    try:
        # 1) Ejecutar skill: añadir mago del día (un mago nuevo + regenerar HTML)
        ...
        try:
            ...
            add_result = run_magia("add") or ""
            # Éxito solo si el skill añadió un mago (texto "actualizada" y sin "no se pudo descubrir")
            add_ok = (
                "actualizada" in add_result.lower()
                and "no se pudo descubrir" not in add_result.lower()
            )
            if not add_ok:
                logger.warning("Magia add no añadió mago hoy (Ollama o skill): %s", add_result[:300])
        except Exception as e:
            logger.warning("Magia skill add failed: %s", e)
        # 2) Solo publicar en GitHub si se añadió el mago (así no marcamos el día sin sumar uno)
        if not add_ok:
            return False
        ...
        if not ok:
            ...
            return False
        logger.info("Magia publicada a las 10:00 (mago del día añadido): %s", url_or_err)
        ...
        # Marcar que ya se ejecutó hoy (solo si añadimos mago y publicamos)
